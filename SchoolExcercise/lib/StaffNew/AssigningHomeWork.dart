...
import 'package:school/StaffNew/ViewHomeWork.dart';
import 'package:school/main.dart';

class AssigningHomeWork extends StatefulWidget {
  AssigningHomeWork({Key key}) : super(key: key);

  @override
  _AssigningHomeWorkState createState() => _AssigningHomeWorkState();
}

class _AssigningHomeWorkState extends State<AssigningHomeWork> {
          ...
          IconButton(
              ...
              onPressed: () async {
                ...
                await Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => ViewHomeWork()),
                  // ViewStaffDetails()),
                );
              })
        ],
                ...
                onPressed: () async {
                  ...
                  FormData formData = new FormData.fromMap({
                    ...
                    /* [ await MultipartFile.fromFile("${files[0].path}",
                            ...
                            await MultipartFile.fromFile("${files[1].path}",
                    ...
                    /*  [ await MultipartFile.fromFile(files[0].path,
                            filename: path.basename(files[0].path)), await MultipartFile.fromFile(files[1].path,
                            filename: path.basename(files[1].path))] */
                    /* fileArrays, */
                  });
                  response = await dio.post(url, data: formData);
